...
from enroute.environments.dataset import TaskDataset, task_content_hash
from enroute.environments.env import Environment
from enroute.environments.policy import Policy
from enroute.environments.rollout import Rollout
from enroute.environments.runtime import LocalRuntime, Runtime, runtime_fingerprint
from enroute.environments.task import TaskData
from enroute.tracing.schema import Trace, TraceKind
...
class Benchmark:
    ...
    def __init__(
        ...
        *,
        ...
        self.client: Enroute | None = client
    ...
    def from_policies(
        ...
        *,
        ...
        Returns:
            A benchmark configured without an Enroute client.
        """
        if not policies:
        ...
        def _run(job: tuple[CaseKey, TaskData]) -> tuple[CaseKey, _JobOutcome]:
            ...
            try:
                ...
                if self.client is None:  # pragma: no cover - constructor invariant
                    raise RuntimeError("model benchmark requires an Enroute client")
                if runtime is None:
