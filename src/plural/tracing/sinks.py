...
Examples:
    ...
    >>> from plural.tracing.sinks import JSONLSink
    >>> import tempfile
...
class OTelSink:
    ...
    def __init__(self, tracer_name: str = "plural") -> None:
        try:
            from opentelemetry import trace as otel_trace
        except ImportError as exc:  # pragma: no cover
            raise ImportError(
                "OTelSink requires the 'otel' extra: pip install plural[otel]"
            ) from exc
    ...
    def write(self, trace: Trace) -> None:
        ...
        """
        with self._tracer.start_as_current_span(f"plural.trace.{trace.trace_id}") as parent:
            parent.set_attribute("plural.trace_id", trace.trace_id)
            if trace.environment:
                parent.set_attribute("plural.environment", trace.environment)
            if trace.task_id:
                parent.set_attribute("plural.task_id", trace.task_id)
            from plural.tracing.schema import LLMCall
            ...
            for step in trace.steps:
                ...
                with self._tracer.start_as_current_span("gen_ai.chat") as span:
                    ...
                    if step.latency_ms is not None:
                        span.set_attribute("plural.latency_ms", step.latency_ms)
                    if step.error:
