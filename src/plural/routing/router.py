...
from plural.routing.policies import Explicit, ModelRoute, RoutingPolicy
from plural.types import ChatRequest, ChatResponse, StreamChunk, Usage
...
class Router:
    ...
    Args:
        providers: Mapping of provider slug to :class:`~plural.providers.base.Provider`.
        catalog: Model catalog for cost and provider lookup.
        policy: Routing policy. Defaults to :class:`~plural.routing.policies.Explicit`.
    ...
    def _routes(self, request: ChatRequest) -> list[ModelRoute]:
        ...
        # Hosted gateway mode: one ``plural`` provider serves every author/slug.
        if "plural" in self.providers:
            return [
                ModelRoute(
                    model=route.model,
                    provider="plural",
    ...
    def _get_provider(self, route: ModelRoute) -> Provider:
        provider = self.providers.get(route.provider)
        if provider is None and "plural" in self.providers:
            provider = self.providers["plural"]
        if provider is None:
            raise ConfigurationError(
    ...
    def chat(self, request: ChatRequest) -> tuple[ChatResponse, list[AttemptRecord]]:
        ...
        Raises:
            PluralError: When all routes fail.
        """
        attempts: list[AttemptRecord] = []
        last_error: PluralError | None = None
    ...
    def stream(
        ...
        for route in routes:
            ...
            try:
                ...
                from plural.types import Choice, Message
