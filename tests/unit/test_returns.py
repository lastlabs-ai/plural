...
def _episode(*actions: str, reward: float | None = 1.0) -> Trace:
    ...
    for name in actions:
        trace.add_turn(parsed_action=[ParsedAction(name=name)])
    return trace


def test_sparse_outcome_returns() -> None:
    trace = _episode("search", "read", "answer")
    assert trace.turn_rewards(source="outcome") == [0.0, 0.0, 1.0]
...
def test_credit_decision_then_events_returns() -> None:
    ...
    event = trace.credit(0.8, name="likes", reason="overnight", turn_index=1)
    assert event.name == "likes"
    assert event.value == 0.8
    assert trace.turns()[1].reward_events[-1].name == "likes"
    assert trace.turn_rewards(source="events") == [0.0, 0.8]
...
def test_credit_negative_index_and_both() -> None:
    ...
    trace.credit(0.4, name="reviewer", turn_index=-1)
    assert trace.turn_rewards(source="both") == pytest.approx([0.0, 1.4])
    ...
    for source in ("outcome", "events", "both"):
        assert [transition.reward for transition in trace.transitions(source=source)] == (
            trace.turn_rewards(source=source)
        )
...
def test_credit_out_of_range() -> None:
    ...
    with pytest.raises(IndexError):
        trace.credit(1.0, turn_index=0)
    trace.add_turn(parsed_action=[ParsedAction(name="a")])
    with pytest.raises(IndexError):
        trace.credit(1.0, turn_index=3)
